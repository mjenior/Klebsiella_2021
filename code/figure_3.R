...
clinical_only <- setdiff(colnames(clinical), colnames(laboratory))
laboratory_only <- setdiff(colnames(laboratory), colnames(clinical))
...
laboratory_only <- laboratory[, laboratory_only]
rm(core_rxns)
...
vioplot(laboratory_biomass, clinical_biomass, col=c('white', 'darkcyan'), 
...
par(xpd=TRUE)
text(x=c(1,2), y=-0.45, labels=c('Laboratory','Clinical'), cex=1.2, srt=45)
...
points(x=clinical_points$MDS1, y=clinical_points$MDS2, bg=alpha('darkcyan',0.8), pch=21, cex=1.7)
...
points(x=laboratory_points$MDS1, y=laboratory_points$MDS2, bg=alpha('white',0.8), pch=21, cex=1.7)
legend('topleft', legend='Core Metabolism', bty='n', pt.cex=0, cex=1.3)
legend('topright', legend=c('Laboratory strains','Clinical isolates'), 
       pt.bg=c('white', 'darkcyan'), pch=21, pt.cex=1.6, cex=1.1, box.lwd=1.7)
legend('bottomright', legend=as.expression(bquote(paste(italic('p'),'-value = 0.001 ***'))), 
       bty='n', pt.cex=0, cex=0.8)
box()
par(xpd=TRUE)
